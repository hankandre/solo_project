...
userApp.controller('UserController', ['$scope', 'UserService', '$mdDialog', '$filter', function ($scope, UserService, $mdDialog, $filter) {
  ...
  $scope.date = new Date();
  $scope.minDate = new Date($scope.date.getFullYear(), $scope.date.getMonth() - 1, $scope.date.getDate());
  $scope.maxDate = new Date($scope.date.getFullYear(), $scope.date.getMonth(), $scope.date.getDate());
